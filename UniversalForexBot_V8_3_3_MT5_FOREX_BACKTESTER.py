...
BOT_PATH = Path(__file__).with_name("UniversalForexBot_V8_3_3_MT5_FOREX.py")
...
APP_VERSION="V8.3.3-FOREX-BT"
APP_TITLE="Universal Forex Bot V8.3.3 — MT5 Forex Strategy Backtester"
...
DEFAULTS={
    ...
    "daily_loss_pct":3.0,"daily_profit_pct":0.0,"no_same_candle":True,"hold_until_all_reverse":True,
}
...
def backtest(df,c,progress=None):
    x=build_frame(df,c); equity=float(c['capital']); start=equity; peak=equity; daily_start=equity; daily_day=None; trades=[]; pos=None; cooldown=0; last_entry=-999; max_dd=0.0
    spread=float(c['spread_pips'])*0.0001; slip=float(c['slippage_pips'])*0.0001
    for i in range(250,len(x)-1):
        ts=x.datetime.iloc[i]
        if daily_day!=ts.date(): daily_day=ts.date(); daily_start=equity
        ...
        if pos:
            ...
            if exit_px is not None:
                ...
                equity+=pnl; equity-=float(c['commission_per_lot'])*pos['orig_qty']; trades.append({**pos,'exit':exit_px,'reason':reason,'pnl':pnl}); pos=None; cooldown=int(c['cooldown_bars'])
    ...
    return {'starting_equity':start,'ending_equity':equity,'net_pnl':net,'return_pct':net/start*100,'trades':len(trades),'wins':wins,'losses':losses,'win_rate':wins/len(trades)*100 if trades else 0,'max_drawdown_pct':max_dd*100,'trades_detail':trades}
