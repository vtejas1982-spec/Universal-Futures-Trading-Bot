...
BOT_PATH = Path(__file__).with_name("UniversalForexBot_V8_4_0_MT5_FOREX_EVIDENCE_HARDENED.py")
...
DEFAULTS={
    ...
    "signal_mode":"ADAPTIVE_EVIDENCE","min_score":1,"adaptive_edge":0.18,"adaptive_min_weight":3.5,"evidence_min_families":2,"evidence_family_min_score":0.35,"evidence_require_trend":True,"evidence_require_independent":True,
...
def signal_at(x,i,c):
    ...
    b,s,bs,ss=fx.StrategyEngine.decide_signal(mods,c['signal_mode'],int(c['min_score']),ap,vp,dp,mb,ms,float(c['adaptive_edge']),float(c['adaptive_min_weight']),int(c.get('evidence_min_families',2)),float(c.get('evidence_family_min_score',0.35)),bool(c.get('evidence_require_trend',True)),bool(c.get('evidence_require_independent',True)))
