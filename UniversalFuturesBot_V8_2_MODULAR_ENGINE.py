...
class StrategyEngine:
    ...
    def decide_signal(directional_modules, signal_mode, min_score,
        ...
        if signal_mode in ("SCORE", "2_SIGNALS", "3_SIGNALS", "4_SIGNALS"):
            return (
                count > 0 and buy_score >= min_score and buy_score > sell_score,
                count > 0 and sell_score >= min_score and sell_score > buy_score,
                buy_score, sell_score,
            )
        if signal_mode != "STRICT_ALL_FILTERS":
            raise ValueError(f"Unknown signal mode: {signal_mode}")
        strict_buy = bool(modules) and all(bool(bull) and not bool(bear) for _, bull, bear in modules)
        strict_sell = bool(modules) and all(bool(bear) and not bool(bull) for _, bull, bear in modules)
        return (
            strict_buy and bool(atr_pass) and bool(vol_pass) and bool(adx_pass) and bool(mtf_pass_bull),
            strict_sell and bool(atr_pass) and bool(vol_pass) and bool(adx_pass) and bool(mtf_pass_bear),
            buy_score, sell_score,
        )
...
class UniversalFuturesBotGUI:
    ...
    def _validate_v82_preflight(self):
            ...
            if self.symbol and grid_mode in ("LONG_GRID", "SHORT_GRID", "NEUTRAL_GRID"):
                if self.grid_state.get("active"):
                    ...
                    if not self.fetch_position(self.symbol):
                        self.cancel_all_open_orders(self.symbol)
                    else:
                        self.log(
    ...
    def _decide_signal(
        ...
        return StrategyEngine.decide_signal(
            ...
            atr_pass, vol_pass, adx_pass, mtf_pass_bull, mtf_pass_bear
        )
