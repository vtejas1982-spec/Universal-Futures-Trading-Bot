"""
V8.3.3 UNIVERSAL FUTURES BACKTESTER — HARDENED ADAPTIVE STRATEGY-PARITY EDITION
===============================================================
...
Important:
    This V8.3.3 backtester mirrors the live strategy/indicator contract. The
...
LIVE_STRATEGY_PARITY_VERSION = "V8.3.3"
...
def run_backtest(df,cfg,logger=None):
    ...
    # Normalize raw OHLCV input once. Public callers may provide millisecond time
    # without a prebuilt datetime or strategy-indicator frame.
    df = df.copy()
    if "datetime" not in df.columns:
        if "time" not in df.columns:
            raise ValueError("Backtest data requires either datetime or millisecond time.")
        df["datetime"] = pd.to_datetime(df["time"], unit="ms", utc=True)
    else:
        df["datetime"] = pd.to_datetime(df["datetime"], utc=True)
    required_columns = {"trend", "ema", "vol_ma"}
