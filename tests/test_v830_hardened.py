...
def test_adaptive_grid_smoke():
    ...
    open_ = np.r_[100.0, close[:-1]]
    high = np.maximum(open_, close) * (1 + rng.uniform(0, .002, n))
    low = np.minimum(open_, close) * (1 - rng.uniform(0, .002, n))
    vol = rng.lognormal(10, .2, n)
    df = pd.DataFrame({'time': np.arange(n)*900000, 'open': open_, 'high': high, 'low': low, 'close': close, 'vol': vol})
    df['datetime'] = pd.to_datetime(df.time, unit='ms', utc=True)
    cfg = bt.validate_config(dict(bt.DEFAULTS, use_mtf=False, signal_mode='ADAPTIVE_SCORE',
                                  adaptive_edge=.18, adaptive_min_weight=3.5,
                                  grid_mode='NEUTRAL_GRID', grid_trend_filter='SCORE',
                                  grid_score_min=1, warmup=250, max_trades=0))
    x = bt.build_strategy_columns(df, cfg)
    trades, metrics, equity, events = bt.run_backtest(x, cfg)
    assert isinstance(metrics, dict) and 'net_pnl' in metrics
...
if __name__ == '__main__':
    tests = [test_compile, test_contracts, test_adaptive_parity, test_backtest_smoke, test_adaptive_grid_smoke, test_safety_contracts_static]
    for t in tests:
